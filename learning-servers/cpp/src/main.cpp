...
int main(int, char **argv) {
  ...
  svr.Get("/hello", [](const httplib::Request &, httplib::Response &res) {
    res.set_content("GET hello from C++!\n", "text/plain; charset=utf-8");
  });
  svr.Post("/hello", [](const httplib::Request &, httplib::Response &res) {
    res.set_content("POST hello from C++!\n", "text/plain; charset=utf-8");
  });
  svr.Get("/pdf", [](const httplib::Request &, httplib::Response &res) {
    res.set_content(build_pdf("Hello from C++! PDF benchmark."),
                    "application/pdf");
  });
  svr.Post("/validate",
           [](const httplib::Request &req, httplib::Response &res) {
             bool ok = false;
             try {
               ok = validate_payload(json::parse(req.body));
             } catch (...) {
               ok = false;
             }
             res.status = ok ? 200 : 400;
             res.set_content(ok ? "{\"valid\":true}" : "{\"valid\":false}",
                             "application/json");
           });
